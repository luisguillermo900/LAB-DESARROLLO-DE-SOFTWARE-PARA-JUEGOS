...
public class FlockUnit : MonoBehaviour
{
    ...
    {
        if (assignedFlock.Target == null)
            return Vector3.zero;

        Vector3 directionToTarget = assignedFlock.Target.position - myTransform.position;
        if (IsInFOV(assignedFlock.Target.position)) // Sólo si el objetivo está en el campo de visión
        {
            directionToTarget = directionToTarget.normalized; // Vector normalizado hacia el objetivo
        }
